...
var phoneLine = `${BUSINESS.sameDayReply} ${BUSINESS.hoursDisplay}`;
var cityLinks = [
...
var corePage = (page) => ({
  ...page,
  answer: page.answer ?? `${page.h1} Blue Tape Sites builds contractor websites for ${BUSINESS.serviceAreaDetail}, with a ${BUSINESS.leadMagnet}, ${BUSINESS.auditTurnaround}, and phone support at ${BUSINESS.phoneDisplay}.`,
  type: "core"
});
var coreSeoPages = [
  corePage({
  ...
  corePage({
    ...
    eyebrow: "Contact",
    summary: `The fastest way to start is to request a free audit or call ${BUSINESS.phoneDisplay}. ${BUSINESS.hoursDisplay}`,
    sections: [
...
var neighborhoodSentence = (city, neighborhood, index) => `${neighborhood}: ${neighborhoodNotes[city.slug]?.[index] ?? `contractors here need pages that connect ${city.nuance.toLowerCase().split(".")[0]} to a fast call path, proof, and service-area clarity.`}`;
var cityPage = (seed) => {
...
var citySeoPages = citySeeds.map(cityPage);
var cityHref = (city) => cityLinks.find((link) => link.label === city)?.href ?? "/service-area";
var industrySeeds = [
...
var industrySeoPages = industrySeeds.map(industryPage);
...
var homePage = {
  ...
  answer: `Blue Tape Sites is a Southern California contractor web design studio that builds phone-first websites for plumbers, electricians, cleaners, and home-service teams. Business owners can request a free 5-minute video audit with a 48-hour turnaround or call ${BUSINESS.phoneDisplay} for same-day reply.`,
  sections: [
    {
...
var testimonialReviews = [
  {
    ...
    quote: "I get most of my work from people needing help right now, so the site has to make sense in a hurry. They cleaned up the offer, made the phone CTA obvious, and the whole thing finally feels like it belongs to a real plumbing outfit instead of some cookie-cutter template."
  },
  {
    name: "Shawn Keller",
    company: "Keller Electric Co.",
    quote: "What I liked was they didn't try to sell me on fancy nonsense. They looked at the page, pointed out where trust was leaking, and fixed the order of things. Better headline, better proof, better mobile layout. Straightforward and worth doing."
  },
  {
    name: "Tina Alvarez",
    company: "Alvarez Cleaning Crew",
    quote: "Most people in my business just need a site that looks clean, answers fast, and doesn't make the company look small-time. That's what this did. It reads better, books better, and I don't feel weird sending customers to it anymore."
  },
  {
    name: "Derek Holcomb",
    company: "Holcomb Garage Door Service",
    quote: "I've dealt with marketing people before and half the time they're hard to pin down. This felt straightforward. They showed me what was off, tightened it up, and left me with a page that's a lot easier for customers to trust."
  }
];
...
var blogSeoPages = [
  {
    ...
    answer: `The Blue Tape Sites blog answers contractor website, local SEO, and AI-search questions for Southern California service businesses that need more calls. Start with a free 5-minute audit or call ${BUSINESS.phoneDisplay}.`,
    sections: [],
    type: "core"
  },
  ...blogPosts.map((post) => ({
...
var caseStudySeoPages = [
  {
    ...
    description: "How we rebuilt Maria's Family Cleaning into a bilingual, trust-first house cleaning site that books 247+ OC families and earned a 5.0 rating across 127+ reviews.",
    ...
    eyebrow: "Case study",
    summary: "A bilingual cleaning company in Orange County needed a website that felt as warm as a referral from your neighbor and as serious as a $150 deep clean.",
    answer: "Maria's Family Cleaning is a Blue Tape Sites case study showing how a bilingual, trust-first cleaning website can turn reviews, pricing, service area, and quote flow into a stronger booking engine for Orange County homeowners.",
    sections: [
      {
      ...
      },
      {
        ...
        body: "247+ families served. 127+ five-star reviews. 12 cities covered. The numbers Maria publishes speak for themselves, and they are the numbers the site was built to make possible."
      },
      {
        title: "Want the same treatment for your trade?",
        body: "Blue Tape Sites builds trust-first websites for cleaners, plumbers, electricians, HVAC, remodelers, and other Southern California service businesses. Request a free audit or call for same-day reply."
      }
    ],
    type: "core",
...
var seoPagesForSitemap = [homePage, ...allSeoPages, ...caseStudySeoPages, ...blogSeoPages];
...
function buildJsonLd(page) {
  ...
  const breadcrumbItems = page.type === "industry" ? ["Home", "Services", page.breadcrumbLabel ?? page.h1] : isCaseStudy ? ["Home", "Examples", "Case Studies", page.breadcrumbLabel ?? page.h1] : ["Home", page.type === "city" ? "Cities" : page.breadcrumbLabel ?? page.h1];
  graph.push({
    "@type": "BreadcrumbList",
    itemListElement: breadcrumbItems.map((name, index) => ({
      ...
      position: index + 1,
      name,
      item: index === 0 ? SITE_URL : index === breadcrumbItems.length - 1 ? canonicalUrl : isCaseStudy && name === "Examples" ? `${SITE_URL}/examples` : isCaseStudy && name === "Case Studies" ? `${SITE_URL}/case-studies` : `${SITE_URL}${page.type === "city" ? "/service-area" : "/services"}`
    }))
  });
  if (isCaseStudy) {
    graph.push({
...
function renderSeoHtml(template, requestUrl) {
  ...
  const page = path2 === "/" ? homePage : getSeoPageByPath(path2) ?? caseStudySeoPages.find((item) => item.path === path2) ?? blogSeoPages.find((item) => item.path === path2);
  if (!page) {
    return template;
  }
  const canonicalUrl = `${SITE_URL}${page.path === "/" ? "/" : page.path}`;
